...
public interface SubscribeRepository extends JpaRepository<SubscribeEntity, Long> {
    ...
    @Query(value = """
        SELECT u.*\s
        FROM users u\s
        INNER JOIN (SELECT subscriber_id FROM subscribe WHERE subscription_id = :user_id) s\s
    ...
    @Query(value = """
        SELECT u.*\s
        FROM users AS u\s
        INNER JOIN (SELECT subscription_id FROM subscribe WHERE subscriber_id = :user_id) AS s\s
